...
struct Teacher{
    ...
    void print_class(){
        cout << "classroom number " << class_number << endl;
    }
    int floor(){
        return floor_level;
    }
...
struct Class{
    ...
    void place_student(Student* s){
        ...
        current_students_in++;
        if ( current_students_in == Cclass) this->can_join_class = false;
        s->print_student();
        cout << " has entered Class " << number_of_class << endl;;
    }
    void place_teacher(Teacher* t){
        teacher = t;
        this->can_join_class = false;
        this->teacher_in = true;
        t->set_state(true);
        teacher->print_teacher();
        cout << "- Teacher has entered the classroom " << number_of_class << "." << endl;
    }
    bool can_join(){
...
struct Corridor{ 
    ...
    void student_allocation(){
        ...
        while( table_of_students[i] != NULL ){
            if( table_of_classes[ (table_of_students[i]->get_class()-1)/6 ]->can_join() ){
                ...
                cout << "Student ";
                table_of_students[i]->print_student();
                cout << "cannot join because ";
                if( table_of_classes[ (table_of_students[i]->get_class()-1)/6 ]->teacher_inside() ){
                    cout << "teacher is inside." << endl;
                } else {
                    cout << "class if full." << endl;
                }
            }
        }
        fix_table();
    ...
    void place_teacher(Teacher *teacher){
        table_of_classes[ teacher->class_no() -1]->place_teacher(teacher);
    }
};
...
struct Floor{ 
    ...
    void place_teacher(Teacher * teacher){
        corridor.place_teacher(teacher);
    }
};
...
struct Stairs{
    ...
    void allocate_students(){
        ...
        while( table_of_students[i] != NULL ){
            ...
            table_of_students[i]->print_student();
            cout << endl;
            cout << "Printing table_of_floors[" << table_of_students[i]->get_class()/6 << "]" << endl;
            if ( table_of_floors[ table_of_students[i]->get_class()/6 - 1]->can_join() ){//if the floor can accept more ppl
                Student *s = table_of_students[i];
                remove_student(s);
    ...
    void place_teacher(Teacher * teacher){
        table_of_floors[ teacher->floor() -1 ]->place_teacher(teacher);
    }
};
...
struct Yard{
    ...
    Yard( const int & Ccla, const int &Ccor, const int & Cstai, const int Cya):
        ...
        can_join_yard(true), current_students_in(0), stairs(Ccla, Ccor, Cstai){
    ...
    bool can_join(){
        return can_join_yard;
    }
    void place_student(Student *s){
        table_of_students[current_students_in] = s;
        current_students_in++;
        if (current_students_in == Cyard) can_join_yard = false;
        s->print_student();
    ...
    void allocate_students(){
        int i = 0;
        while( table_of_students[i] != NULL && stairs.can_join() == true){
            Student *s = table_of_students[i];
            remove_student(s);
            stairs.place_student(s);
        }
        fix_table();
        stairs.allocate_students();
    }
    void print(){
...
class School{
    private:
         ...
         const int Cclass, Ccorr, Cstair, Cyard;
         int current_students_in;
        bool can_join_school;
    public:
        School( const int & Ccla, const int &Ccor, const int & Cstai, const int Cya):
        Cclass(Ccla), Ccorr(Ccor), Cstair(Cstai), Cyard(Cya),
        can_join_school(true), current_students_in(0), yard(Ccla, Ccor, Cstai, Cya){
        ...
        bool can_join(){
            return yard.can_join();
        }
        void place_student(Student *s){
            if (can_join_school == false ) {
                s->print_student();
                cout << "cannot join School right now." << endl;
                return;
            }
            s->print_student();
        ...
        void allocate_students(){
            yard.allocate_students();
        }
        void place_teacher(Teacher* teacher){
            yard.place_teacher(teacher);
        }
        void print(){
...
int main(){
        ...
        Student a7("bob7",1,7,0);
        Student a8("bob8",1,8,0);
        Student a9("bob9",1,9,0);
        Student a10("bob10",1,10,0);
        Student a11("bob11",1,11,0);
        Student a12("bob12",1,12,0);
        Student a13("bob13",1,13,0);
        Student a14("bob14",1,14,0);
        Student a15("bob15",1,15,0);
        Student a16("bob16",1,16,0);
        Student a17("bob17",1,17,0);
        Student a18("bob18",1,18,0);
